...
class NeuralNetworkModel(Model):
    ...
    def plot_history(self):
        ...
        plt.savefig(
            f"{LSTM_PLOTS_PATH}/{self.experiment_name}_nn_training_history_mse_input_window_{self.n_steps}_output_window{self.output_size}_{date_string}.png")  # _{self.architecture_name}
        #plt.show()
        plt.close(fig)
        ###
        ...
        plt.ylabel('MAPE')
        plt.savefig(
            f"{LSTM_PLOTS_PATH}/{self.experiment_name}_nn_training_history_mape_input_window_{self.n_steps}_output_window{self.output_size}_{date_string}.png")  # _{self.architecture_name}
        #plt.show()
        plt.close(fig2)
    ...
    def save_predictions(self, X_test, Y_test, Y_predict, company_name_test, input_period_start_date_test,
        ...
        for i in range(len(Y_test)):  # len of X_test or Y_predict can be also used here
            ...
            date_string = dt.datetime.now().strftime("%Y-%m-%d_%H-%M")
            plt.savefig(
                f"{LSTM_PLOTS_PATH}/{self.experiment_name}_prediction_plot_id_{imageid}_input_window_{self.n_steps}_output_window{self.output_size}_{date_string}.png")  # _{self.architecture_name}
            # plt.show()
            plt.close(fig)
    ...
    def save_results(self, X_test, Y_test, Y_predict, company_name_test, input_period_start_date_test, input_period_end_date_test,
        ...
        df2.to_csv(
            f"{LSTM_MODEL_RESULTS}/{self.experiment_name}_errors_input_window_{self.n_steps}_output_window{self.output_size}_{date_string}.csv")
